...
SH1106_SPI::SH1106_SPI(spi_inst_t* _spi) {
	spi = _spi;
}

void SH1106_SPI::begin(bool invert, uint8_t contrast, uint8_t Vpp) {
	spi_init(spi, 4000 * 1000);
...
size_t SH1106_SPI::write(uint8_t data) {
	...
	uint8_t buf[6] = {ASCII[data - 0x20][0], ASCII[data - 0x20][1], ASCII[data - 0x20][2], ASCII[data - 0x20][3], ASCII[data - 0x20][4]};
	buf[5] = 0x00;
...
void SH1106_SPI::writeLcd(uint8_t dataOrCommand, const uint8_t *data, uint16_t count) {
	...
	spi_write_blocking(spi, data, count);
}
