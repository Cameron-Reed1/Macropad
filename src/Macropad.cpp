...
Macropad::Macropad()
: m_keys(NUM_KEYS, true), m_oled(SPI_PORT), m_state(MacropadState()), m_encoder(ROTA, ROTB, RotaryEncoder::LatchMode::FOUR3), m_pixels(NUM_PIXELS, NEOPIXEL, NEO_GRB + NEO_KHZ800)
{}
...
void Macropad::run()
{
    ...
    if (m_useEncoder) {
		...
		if (m_encoderPressed != m_encoderWasPressed && !m_stateChanged) {
			...
			m_updateOled = m_state.get_update_on_encoder_press();
		}
        if (m_state.get_encoder_position() != m_state.get_encoder_last_postition() && !m_stateChanged) {
            m_updateOled = m_state.get_update_on_encoder_move();
        }
    ...
    if (m_useKeys) {
        ...
        if (m_keys.haveChanged()) {
            m_updateOled = m_state.get_update_on_key_press();
